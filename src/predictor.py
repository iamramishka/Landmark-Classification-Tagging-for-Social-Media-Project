...
class Predictor(nn.Module):
    def __init__(self, model, class_names, mean, std):
        ...
        # is compatible with torchscript, while the latter isn't
        self.transforms = nn.Sequential(
            T.Resize([256], antialias=True),  # Add antialias=True to suppress the warning
            T.CenterCrop(224),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            ...
            # 3. Apply softmax across dim=1 to get probabilities
            x = F.softmax(x, dim=1)
